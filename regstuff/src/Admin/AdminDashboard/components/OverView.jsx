...
export default function Overview() {
  ...
  const cartproducts=useSelector((state)=>state.ordersdatareducer)
  const {allcartdata}=cartproducts
const {data}=userdetails
let sum=0
for(let key of allcartdata){
  sum=sum+((key.price)*(Number(key.quantity)))
}
...
<Box backgroundColor="#fff7cd" ><Heading  as="h3" fontFamily={"sans-serif"} color={"#061b64"} fontWeight="bold" textAlign="center" fontSize={"16px"}> <Circle margin={"auto"} mt="30px" size='50px' bg='tomato' color='white'>
    <ShoppingCartIcon />
  </Circle>Total Orders</Heading><Heading as="h3" fontFamily={"sans-serif"} color={"#061b64"} fontWeight="bold" textAlign="center" fontSize={"16px"}>{typeof allcartdata!=="undefined"&&allcartdata.length}</Heading></Box>
<Box backgroundColor="#ffe7d9" ><Heading  as="h3" fontFamily={"sans-serif"} color={"#061b64"} fontWeight="bold" textAlign="center" fontSize={"16px"}> <Circle margin={"auto"} mt="30px" size='50px' bg='#1877f2' color='white'>
    <CurrencyRupeeIcon />
  </Circle>Total Income</Heading><Heading as="h3" fontFamily={"sans-serif"} color={"#061b64"} fontWeight="bold" textAlign="center" fontSize={"16px"}>{sum&&sum}</Heading></Box>
